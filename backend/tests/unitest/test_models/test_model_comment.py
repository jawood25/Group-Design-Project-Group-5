...
from backend.api.models import Route, Comment  # Adjust the import according to your project structure
...
def create_test_comment(author, body, likes=0, dislikes=0):
    ...
    comment = Comment(author=author, body=body, likes=likes, dislikes=dislikes, date_posted=datetime.datetime.now())
    comment.save()
    return comment
...
def test_comment_methods(test_client, test_case):
    ...
    try:
        if method == "__init__":
            ...
            assert comment.__repr__() == f"{test_case['author']}'s comment"
            ...
            assert retrieved_route == test_route, "get_route method failed to retrieve the correct route."
