...
class TestUserLogin:
    ...
    def test_user_login(self, test_client, test_cases):
        # Use the correct password for login test
        login_password = test_cases.get("wrong_password", test_cases["password"])
        ...
        expected_status = test_cases.get("expected_status")
        if "wrong_password" in test_cases:
